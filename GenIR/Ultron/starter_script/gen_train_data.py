...
parser.add_argument("--scale", default="all", type=str, help="docid method atomic/pq/url")
...
max_seq_length = 128
top_or_rand, scale = args.scale, args.scale
msmarco_or_nq = "nq"
...
def main():
    ...
    for data_name, sample_for_one_doc in target_data:
        ...
        os.system(f"cd {code_dir}/gen_instance/ && python gen_{model}_train_data.py \
            ...
            --pretrain_model_path ../../../../huggingface/{model}-base \
            --data_path {code_dir}/dataset/{msmarco_or_nq}-data/{msmarco_or_nq}-docs-sents.{top_or_rand}.json \
