...
def product_quantization_docid(args, docid_2_idx, idx_2_docid, doc_embeddings, output_path):
    ...
    # save pq model
    with open(output_path.replace(".txt", ".pkl"), "wb") as fw:
        pickle.dump(pq, fw)
...
def url_docid(input_path, output_path):
    ...
    with open(input_path) as fin:
        for doc_index, line in tqdm(enumerate(fin), desc='loading all docids'):
            ...
            url = url.replace("http://", "").replace("https://", "").replace("-", " ").replace("_", " ").replace("?",
                                                                                                                 " ").replace(
                "=", " ").replace("+", " ").replace(".html", "").replace(".php", "").replace(".aspx", "").strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scale", default="all", type=str, help="top/rand_100k/200k/300k.")
    ...
    parser.add_argument("--output_path", default="dataset/encoded_docid/t5_atomic_top_300k.txt", type=str,
                        help='output path')
    parser.add_argument("--pretrain_model_path", default="../../../huggingface/t5-base", type=str,
                        help='bert model path')
    args = parser.parse_args()

    scale = args.scale.split("_")
    ...
    if args.encoding == "atomic":
        ...
        input_doc_path = 'dataset/msmarco-data/msmarco-docs-url.json'
        output_path = f"dataset/encoded_docid/t5_url_{args.scale}.txt"
        url_docid(input_doc_path, output_path)
        for i in range(1, 6):
            input_doc_path = f'dataset/msmarco-data/msmarco-docs-url_{i}.json'
            output_path = f"dataset/encoded_docid/t5_url_{args.scale}_{i}.txt"
            url_docid(input_doc_path, output_path)