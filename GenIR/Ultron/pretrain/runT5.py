...
def evaluate_beamsearch():
    ...
    if os.path.exists(args.test_file_path):
        ...
        for i, testing_data in tqdm(enumerate(test_dataloader)):
            ...
            outputs = model.generate(input_ids, max_length=args.max_docid_length+30, num_return_sequences=args.num_beams, num_beams=args.num_beams, do_sample=False, prefix_allowed_tokens_fn=prefix_allowed_tokens_fn)
                ...
                for docid in batch_output:
                    if args.use_docid_rank == "False":
                        ...
                        try:
                            ...
                            # for key in encode_2_docid.keys():
                            #     if docid2string(docid).startswith(key):
                            #         print("find prefix: ", key)
                            #         print(1)
                            #         exit(0)
                            # for key in encode_2_docid.keys():
                            #     if key.startswith(docid2string(docid)):
                            #         print("find prefix: ", key)
                            #         print(2)
                            #         exit(0)
                        except:
                            print("docid not found: ", docid2string(docid))
                            docid_list = [1]
                            for key in encode_2_docid.keys():
                                if docid2string(docid).startswith(key):
                                    print("find prefix: ", key)
