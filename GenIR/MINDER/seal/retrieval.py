...
class SEALSearcher:
    ...
    DEFAULTS = \
    {
        "backbone": '../../../huggingface/bart-large',
    ...
    def load(cls, fm_index_path, bart_model_path, device="cpu", **params):
        ...
        bart_tokenizer, bart_model = cls.load_bart(
            bart_model_path, 
            device, 
            backbone=params.get('backbone', "../../../huggingface/bart-large"),
...
#
        # keys = {k: (k, self.fm_index.get_count(list(k))) for k, v in zip(keys, vals)}
...
#                            keys[k] = (k, self.fm_index.get_count(list(k)))
