...
class OurTrainer:
    ...
    def train_step(model: Model, batch, gathered=None):
        ...
        if query_outputs.code_logits is not None:
            ...
            # print(
            #     f'!!!!!!!!!!!!!!!!!!{query_outputs.code_logits.view(-1, code_number).size(), batch["ids"][:, 1:].reshape(-1).size()}')
            query_code_loss = F.cross_entropy(query_outputs.code_logits.view(-1, code_number),
                                              batch['ids'][:, 1:].reshape(-1))
            doc_code_loss = F.cross_entropy(doc_outputs.code_logits.view(-1, code_number),
                                            batch['ids'][:, 1:].reshape(-1))
...
def train(config):
    process_kwargs = DDPK(find_unused_parameters=True)
    accelerator = Accelerator(gradient_accumulation_steps=1, kwargs_handlers=[process_kwargs])
            ...
            with accelerator.accumulate(model):
                ...
                if in_batch_size != 1 and step > (epoch + 1) * epoch_step:
                    epoch, last_checkpoint = safe_save(accelerator, model, save_path, epoch, end_epoch=end_epoch,
                                                       save_step=save_step,
                                                       last_checkpoint=last_checkpoint, config=config)
                if epoch >= end_epoch:
                    break
        if in_batch_size == 1:
...
def test(config, epoch):
    ...
    code_length = config.get('code_length', 1)
    prev_id = config.get('prev_id', None)
    prev_id = 'out/model-3-pre/9.pt.code'
    save_path = config.get('save_path', None)
    batch_size = 32
    epochs = config.get('epochs', 100)
...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='../../../../huggingface/t5-base')
    parser.add_argument('--code_num', type=int, default=512)
    parser.add_argument('--max_length', type=int, default=3)
    parser.add_argument('--train_data', type=str, default='dataset/D0/train.json')
    parser.add_argument('--dev_data', type=str, default='dataset/D0/dev.json')
    parser.add_argument('--corpus_data', type=str, default='dataset/D0/corpus_lite.json')
...
def main():
    args = parse_args()
    config = f'configs/config_{args.start}_{args.flag}.json'
    ...
    # for loop in range(start, args.max_length):
    #     if flag == 1:
    #         checkpoint = config['checkpoint']
    #         config['save_path'] = args.save_path + f'-{loop + 1}-pre'
    #         config['code_length'] = loop + 1
    #         config['prev_model'] = checkpoint
    #         config['test'] = f'{checkpoint}.code' if checkpoint is not None else None
    #         config['epochs'] = 1 if loop == 0 else 10
    #         config['loss_w'] = 1
    #         checkpoint = train(config)
    #         ckpt_d = {0: 'out/model-1-pre/0.pt', 1: 'out/model-2-pre/9.pt', 2: 'out/model-3-pre/9.pt'}
    #         config['checkpoint'] = ckpt_d[loop]
    #         save_config(config)
    #         exit(0)
    #         # test_dr(config)
    #
    ...
    #         config['loss_w'] = 2
    #         checkpoint = train(config)
    #         ckpt_d = {0: 'out/model-1/199.pt', 1: 'out/model-2/199.pt', 2: 'out/model-3/199.pt'}
    #         config['checkpoint'] = ckpt_d[loop]
    #         save_config(config)
    #         exit(0)
    #
    ...
    if args.start == 3:
        ...
        # config['code_length'] = loop + 1
        # config['prev_model'] = checkpoint
        # add_last(f'{checkpoint}.code', config['code_num'], f'{checkpoint}.code.last')
